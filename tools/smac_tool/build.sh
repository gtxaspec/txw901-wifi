...
# #
 ...
 if [ ! -d  hostapd-2.9 ]; then
	 ...
	 sed -i "s/\/dev\/random/\/dev\/urandom/g" hostapd-2.9/src/crypto/random.c
 fi
...
# #
 ...
 if [ ! -d  wpa_supplicant-2.9 ]; then
	 ...
	 sed -i "s/\/dev\/random/\/dev\/urandom/g" wpa_supplicant-2.9/src/crypto/random.c
 fi
...
echo "Build Test_App ..."
[ ! -e ../test_app/bin ] && mkdir ../test_app/bin
cd ../test_app;make CC=${COMPILER}gcc AR=${COMPILER}ar STRIP=${COMPILER}strip hgics hgpriv;cd -
cp -fv ../test_app/bin/*  bin/
