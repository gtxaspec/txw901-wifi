...
extern char *hgic_strchr(char *str, char s, int index);
...
static int hgics_wpacli_get_rssi()
{
    ...
    if (hgic_do_system("wpa_cli signal_poll", buff, 124) > 0) {
...
static void hgics_parse_wpacli_status(char *str)
{
    ...
    ptr = strstr(str, "\nssid=");
    if (ptr) {
        hgic_strcpy(g_hgrelay.wpastatus.ssid, ptr + 6, 32);
    }

    ptr = strstr(str, "pairwise_cipher=");
    if (ptr) {
        hgic_strcpy(g_hgrelay.wpastatus.pairwise_cipher, ptr + 16, 10);
    }

    ptr = strstr(str, "group_cipher=");
    if (ptr) {
        hgic_strcpy(g_hgrelay.wpastatus.group_cipher, ptr + 13, 10);
    }

    ptr = strstr(str, "key_mgmt=");
    if (ptr) {
        hgic_strcpy(g_hgrelay.wpastatus.key_mgmt, ptr + 9, 10);
    }
...
static void hgics_check_wpascan_result()
{
    ...
    if (buff) {
        if (hgic_do_system("wpa_cli scan_results", buff, SCAN_RESULT_LEN - 1) > 0) {
            token = strtok(buff, "\n");
            token = strtok(NULL, "\n");
            while (token != NULL) {
                ssid   = hgic_strchr(token, '\t', 4);
                signal = hgic_strchr(token, '\t', 2);
                if (ssid && signal && hgics_is_suitable_ap(ssid)) {
                    rssi = atoi(signal);
                    if (hgics_is_ap(ssid)) {
                        if (rssi > max_rssi) {
                            hgic_strcpy(max_ssid, ssid, 32);
                            max_rssi = rssi;
                        }
                    } else {
                        if (rssi > max_r_rssi) {
                            hgic_strcpy(max_r_ssid, ssid, 32);
                            max_r_rssi = rssi;
                        }
                    }
                }
...
static void hgics_check_signal()
{
    ...
    if (g_hgrelay.hwscan_running == 0 && hgics_wpacli_get_rssi() < g_hgrelay.rssi_th &&
        ...
        if (!hgic_iwpriv_set_hwscan("wlan0", 100, 5, BIT(chan - 1), 1)) {
            g_hgrelay.hwscan_time    = t;
            g_hgrelay.hwscan_running = 1;
        }
    }
#else
    if(hgics_wpacli_get_rssi() < g_hgrelay.rssi_th){
...
void hgics_relay_check_status(void)
{
    ...
    if (hgic_do_system("wpa_cli status", buff, 256) > 0) {
