...
int hgic_do_system(char *cmd, char *buff, int size)
{
    int ret  = 0;
    FILE *fp = popen(cmd, "r");
    if (fp == NULL) {
...
int hgic_iwpriv_do(char *cmd, char *in, int in_len, char *out, int out_len)
{
    ...
    if (ret > 0) {
        ...
        if (ret > 0 && out && out_len) {
            len = ret <= (out_len-1) ? ret : (out_len-1);
            memcpy(out, buff + 4, len);
            out[len] = 0;
        }
    }
    ...
    free(buff);
    if (ret < 0) {
        printf("hgic_iwpriv_write fail, ret=%d, cmd:%s, out_len=%d\r\n", ret, cmd, out_len);
    }
    return ret;
}
...
int hgic_iwpriv_get_bssid(char *ifname, char *mac)
{
    ...
    if(hgic_iwpriv_get_bytes(ifname, "bssid", buf, sizeof(buf)) > 0){
        if (7 == sscanf(buf, MACSTR",%d", &tmp[0], &tmp[1], &tmp[2], &tmp[3], &tmp[4], &tmp[5], &tmp[6])) {
            mac[0] = (unsigned char)tmp[0];
            mac[1] = (unsigned char)tmp[1];
            mac[2] = (unsigned char)tmp[2];
            mac[3] = (unsigned char)tmp[3];
...
int hgic_iwpriv_blenc_set_coexist(char *ifname, int coexist, int dec_duty)
{
    char str[32];
    sprintf(str, "COEXIST_EN:%d,%d", coexist, dec_duty);
    return hgic_iwpriv_set_bytes(ifname, "blenc", str, strlen(str));
}
...
int hgic_iwpriv_blenc_set_advdata(char *ifname, char *adv_data, int len)
{
    ...
    if(len > 31){
        printf("** BLE adv_data max size: 31 **\r\n");
        return -1;
    }
...
{  
    ...
    if(len > 31){
        printf("** BLE scan_resp max size: 31 **\r\n");
        return -1;
    }
...
int hgic_iwpriv_blenc_set_ble_llpkt_len(char *ifname, int length)
{
    char str[32];
    sprintf(str, "LLPKT_LEN:%d", length);
    return hgic_iwpriv_set_bytes(ifname, "blenc", str, strlen(str));
}
...
{
    return hgic_iwpriv_get_bytes(ifname, "fwinfo", (char *)fwifno, sizeof(struct hgic_fw_info));
}
...
int hgic_iwpriv_set_wkdata_mask(char *ifname, char offset/*start from ether hdr*/, char *mask, int mask_len)
{
    char buff[64];
...
{
    return hgic_iwpriv_get_bytes(ifname, "dhcpc_result", (char *)result, sizeof(struct hgic_dhcp_result));
}
...
int hgic_iwpriv_get_sta_info(char *ifname, char *mac_addr, struct hgic_sta_info *stainfo)
{
    ...
    return hgic_iwpriv_do(cmd, 0, 0, (char *)stainfo, sizeof(struct hgic_sta_info));
}
...
int hgic_iwpriv_set_pool_maxusage(char *ifname, int tx_maxusage, int rx_maxusage)
{
    return hgic_iwpriv_set_ints(ifname, "pool_maxusage", 2, tx_maxusage, rx_maxusage);
}

int hgic_iwpriv_set_icmp_mntr(char *ifname, int enable)
{
    return hgic_iwpriv_set_int(ifname, "icmp_mntr", enable);
}

int hgic_iwpriv_set_passwd(char *ifname, char *passwd)
{
    return hgic_iwpriv_set_bytes(ifname, "passwd", passwd, strlen(passwd));
}

int hgic_iwpriv_set_rpasswd(char *ifname, char *passwd)
{
    return hgic_iwpriv_set_bytes(ifname, "rpasswd", passwd, strlen(passwd));
}

int hgic_iwpriv_get_chip_uuid(char *ifname, char uuid[6])
{
    return hgic_iwpriv_get_bytes(ifname, "uuid", uuid, 6);
}
...
int hgic_iwpriv_set_throughput(char *ifname, int enable)
{
    return hgic_iwpriv_set_int(ifname, "throughput", enable);
}

int hgic_iwpriv_set_mem_recycle(char *ifname, int enable)
{
    return hgic_iwpriv_set_int(ifname, "mem_recycle", enable);
}

int hgic_iwpriv_set_host_monitor(char *ifname, int enable)
{
    return hgic_iwpriv_set_int(ifname, "host_mntr", enable);
}

int hgic_iwpriv_set_dma_scatter(char *ifname, int scatter_size)
{
    return hgic_iwpriv_set_int(ifname, "dma_scatter", scatter_size);
}

int hgic_iwpriv_set_sleep_roaming(char *ifname, int enable, int rssi)
{
    return hgic_iwpriv_set_ints(ifname, "sleep_roaming", 2, enable, rssi);
}

int hgic_iwpriv_set_edca_max(char *ifname, struct hgic_txq_param *param)
{
    return hgic_iwpriv_set_bytes(ifname, "edca_max", (char *)param, sizeof(struct hgic_txq_param));
}

int hgic_iwpriv_set_slot_time(char *ifname, int slot_time)
{
    return hgic_iwpriv_set_int(ifname, "slot_time", slot_time);
}
