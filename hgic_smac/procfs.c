...
static int hgics_status_show(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, "hgics status:"HGIC_VERSION"-"SVN_VERSION"\r\n");
    seq_printf(seq, "    FW_STATE:%d, SOFT_FC:%d, STATE:0x%lx, BUS FLAGS:0x%lx, TXWND:%d, DETECT_TMR:%d\r\n", 
                hg->fw_state, hg->soft_fc, hg->state, hg->bus->flags, atomic_read(&hg->txwnd), hg->status.detect_tmr);
                                  ...
                                  size_t count, loff_t *data)
{
    ...
    if (count <= 0 || test_bit(HGICS_STATE_REMOVE, &hg->state)) {
        return -EINVAL;
    }
    ...
    if (buf) {
        ...
        if (cmd == NULL) {
            ret = 0;
            if(count < 1024){
                hgic_err("invalid cmd\r\n");
                copy_to_user((void *)buffer, (const void *)&ret, 4);
                return count;
            }

            wrqin.u.data.pointer = buf;
            wrqin.u.data.length  = count - 4;
            ret = hgic_iwpriv_dump(&hg->ctrl, &wrqin);
            if (ret > 0) {
                if (copy_to_user((void *)(buffer + 4), buf, ret)) {
                    kfree(buf);
            ...
            //hgic_err("**Empty CMD**, ret=%d, count=%d\r\n", ret, count);
            kfree(buf);
            return count;
        }
