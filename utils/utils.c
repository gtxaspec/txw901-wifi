...
void hgic_hdr_format(struct hgic_hdr *hdr, u8 tx)
{
    if(tx){
        hdr->magic  = cpu_to_le16(hdr->magic);
        hdr->length = cpu_to_le16(hdr->length);
        hdr->cookie = cpu_to_le16(hdr->cookie);
    }else{
...
{
    int ret = 0;
    struct umac_config *cfg = umac_configs();
