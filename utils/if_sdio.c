...
typedef struct mmc_command mmc_command_t;
typedef struct sdio_device_id sdio_device_id_t;
typedef struct sdio_driver sdio_driver_t;
#define FUNC_NUM(f)       ((f)->num)
#define DEVID_ID(id)      ((id)->device)
#define DEVID_VENDOR(id)  ((id)->vendor)
#endif
...
#define HGIC_PKT_MAX_LEN      (2*1024)
...
#ifndef SDIO_DMA_ALIGN
#define SDIO_DMA_ALIGN (4)
#endif

static hgic_probe probe_hdl = NULL;
static u32 max_pkt_len = 10 * HGIC_PKT_MAX_LEN;
...
const sdio_device_id_t hgic_sdio_wdev_ids[] = {
...
static inline void hgic_mmc_set_timing(sdio_func_t *func, unsigned int timing)
{
    func->card->host->ios.timing = timing;
    func->card->host->ops->set_ios(func->card->host, &func->card->host->ios);
}
static inline void hgic_mmc_set_bus_width(sdio_func_t *func, unsigned int width)
{
    func->card->host->ios.bus_width = width;
    func->card->host->ops->set_ios(func->card->host, &func->card->host->ios);
}
static inline void hgic_mmc_set_clock(sdio_func_t *func, unsigned int hz)
{
...
static inline int hgic_mmc_send_cmd(sdio_func_t *func, mmc_command_t *cmd, int retries)
{
    return mmc_wait_for_cmd(func->card->host, cmd, retries);
}
static inline void hgic_mmc_set_blk_size(sdio_func_t *func, unsigned int blk_size)
{
...
static int hgic_mmc_io_rw_direct(sdio_func_t *func, int write, unsigned fn,
...
static int hgic_sdio_reset(sdio_func_t *func)
{
    int ret;
    u8 abort;
...
static int hgic_sdio_go_idle(sdio_func_t *func)
{
    int err;
    mmc_command_t cmd = {0};
...
static int hgic_mmc_send_if_cond(sdio_func_t *func, u32 ocr)
{
    mmc_command_t cmd = {0};
    int err;
...
static int hgic_send_io_op_cond(sdio_func_t *func, u32 ocr, u32 *rocr)
{
    mmc_command_t cmd = {0};
    int i, err = 0;
...
static int hgic_sdio_get_card_addr(sdio_func_t *func, u32 *rca)
{
    int err;
    mmc_command_t cmd = {0};
...
static u32 hgic_sdio_set_highspeed(sdio_func_t *func, int enable)
{
    int ret;
    u8 speed;
...
static int hgic_sdio_disable_cd(sdio_func_t *func)
{
    int ret;
    u8 ctrl;
...
static int hgic_sdio_set_wire_width(sdio_func_t *func)
{
    int ret;
    u8 ctrl;
...
static u32 hgic_sdio_select_card(sdio_func_t *func, u32 rca)
{
    int err;
    mmc_command_t cmd = {0};
...
static int hgic_sdio_int_enable(sdio_func_t *func, u8 enable)
{
    u8 temp;
    u8 dat;

    if (enable) {
        dat = (1 << FUNC_NUM(func)) | 0x1;
    } else {
        dat = 0x1;
    }
...
static int hgic_mmc_spi_set_crc(sdio_func_t *func, int use_crc)
{
    mmc_command_t cmd = {0};
    int err;
...
static int hgic_sdio_reinit_card(sdio_func_t *func)
{
...
static int hgic_sdio_readb(sdio_func_t *func, u32 addr, int *err)
{
    u8  val   = 0;
    int retry = 2;
...
static void hgic_sdio_writeb(sdio_func_t *func, u8 b, u32 addr, int *err)
{
    int retry = 4;
    do {
...
static int hgic_sdio_copy_fromio(sdio_func_t *func, u8 *dest, u32 addr, int count)
{
    int  err = 0;
...
static int hgic_sdio_copy_toio(sdio_func_t *func, u32 addr, u8 *src, int count)
{
    int  err = 0;
...
static int hgic_sdio_abort(sdio_func_t *func)
{
    int err_ret = 0;
    sdio_func_t func0;
...
static int hgic_sdio_read_cccr(sdio_func_t *func, u8 *pending)
{
    int ret = 0;
    sdio_func_t func0;
    memcpy(&func0, func, sizeof(func0));
...
static u32 hgic_sdio_int_status(struct hgic_sdio *sdiodev, sdio_func_t *func)
{
    int ret = 0;
    u8 pending = 0;
    u8 int_status1 = 0;
...
static void hgic_sdio_interrupt(sdio_func_t *func)
{
    int ret = 0;
    u32 len = 0;
    int read_more = 0;
    ...
    if (sdiodev == NULL || (sdiodev->status & SDIO_STATUS_STOP) || test_bit(HGIC_BUS_FLAGS_SLEEP_RESP, &sdiodev->bus.flags)) {
        return;
    }
    ...
    do {
        ...
        if (int_status & SDIO_INIT_STATUS_DATA_READY) {
            ...
            if (len == 0xFFFFFFFF) {
                ...
                rxbuf = PTR_ALIGN(sdiodev->rxbuf, SDIO_DMA_ALIGN);
                ret = hgic_sdio_copy_fromio(sdiodev->func, rxbuf, sdiodev->data_addr, len);
                if (ret) {
                    hgic_sdio_abort(func);
                ...
                ret = sdiodev->bus.rx_packet(sdiodev->bus.bus_priv, rxbuf, len);
                if (ret) {
                    hgic_sdio_abort(func);
                }
            } else {
...
static int hgic_sdio_tx_skb(void *bus, struct sk_buff *skb, u8 busy_chk)
{
    int ret  = 0;
    int busy = 1;
    unsigned char *ptr = NULL;
    unsigned char *txbuff = skb->data;
    ...
    if(PTR_ALIGN(txbuff, SDIO_DMA_ALIGN) != txbuff){
        //hgic_err("data addr is unaligned! %p, %d\r\n", txbuff, SDIO_DMA_ALIGN);
        ptr = kmalloc(len + SDIO_DMA_ALIGN, GFP_KERNEL);
        if(ptr == NULL){
            hgic_err("malloc fail! size=%d\r\n", len + SDIO_DMA_ALIGN);
            sdio_release_host(sdiodev->func);
            sdiodev->bus.tx_complete(sdiodev->bus.bus_priv, skb, 0);
            return -1;
        }

        txbuff = PTR_ALIGN(ptr, SDIO_DMA_ALIGN);
        memcpy(txbuff, skb->data, skb->len);
    }

    ret = hgic_sdio_copy_toio(sdiodev->func, sdiodev->data_addr, txbuff, len);
    if(!ret && hgic_is_enter_sleep(skb->data)){
        hgic_err("device enter sleep ...\r\n");
        set_bit(HGIC_BUS_FLAGS_SLEEP, &sdiodev->bus.flags);
    }
    sdio_release_host(sdiodev->func);
...
{
    return hgic_sdio_tx_skb(bus, skb, 1);
}

static int hgic_sdio_tx_ctrl(void *bus, struct sk_buff *skb)
{
    return hgic_sdio_tx_skb(bus, skb, 0);
}

static int hgic_sdio_enable(sdio_func_t *func)
{
    int ret;
...
static int hgic_sdio_probe(sdio_func_t *func, const sdio_device_id_t *id)
{
    int ret = 0;
    struct hgic_sdio *sdiodev = NULL;

    hgic_dbg("Enter, func->num:%d, devid:%x\r\n", FUNC_NUM(func), DEVID_VENDOR(id));
    if (FUNC_NUM(func) != 1) {
        return -ENODEV;
    }
    ...
    sdiodev->rxbuf = kmalloc(max_pkt_len + SDIO_DMA_ALIGN + 4, GFP_KERNEL);
    if (sdiodev->rxbuf == NULL) {
        hgic_err("alloc rxbuf fail, size:%d\r\n", max_pkt_len);
        goto __failed;
    }

    hgic_dbg("new sdio card: vendor:%x, id:%x, PKT_LEN:%d\n", DEVID_VENDOR(id), DEVID_ID(id), max_pkt_len);
    sdiodev->func = func;
    ...
    sdiodev->rx_retry = SDIO_CAP_IRQ(func) ? 0 : (max_pkt_len > 5 * HGIC_PKT_MAX_LEN ? 10 : 5);
...
static void hgic_sdio_remove(sdio_func_t *func)
{
    struct hgic_sdio *sdiodev = NULL;
...
{
    int ret = 0;
    sdio_func_t *func = dev_to_sdio_func(dev);
...
extern sdio_driver_t hgic_sdio_driver;
#else
static sdio_driver_t hgic_sdio_driver = {
    .name     = "hgic_sdio_wlan",
