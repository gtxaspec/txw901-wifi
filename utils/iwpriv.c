...
static int hgic_iwpriv_set_blenc(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    ...
    } else if (memcmp(data, "COEXIST_EN:", 11) == 0) {
        args = data + 11;
        hgic_pick_values(u8, args, vals, 2);
        ret = hgic_fwctrl_set_coexist_en(ctrl, ifidx, vals);
    } else if (memcmp(data, "LLPKT_LEN:", 10) == 0) {
        args = data + 10;
        ret = simple_strtoul(args, 0, 10);
        ret = hgic_fwctrl_set_ble_llpkt_len(ctrl, ifidx, ret);
    } else {
...
static int hgic_iwpriv_enter_sleep(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    ...
    if (vals[0]) {
        ...
        if (test_bit(HGIC_BUS_FLAGS_SLEEP, &ctrl->bus->flags)) {
            ...
            clear_bit(HGIC_BUS_FLAGS_SLEEP_RESP, &ctrl->bus->flags);
            if (ctrl->bus->reinit) {
...
static int hgic_iwpriv_set_pool_maxusage(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    u8 argcnt = 0;
    u8 vals[2];
    hgic_pick_values(u8, data, vals, 2);
    return hgic_fwctrl_set_pool_maxusage(ctrl, ifidx, vals);
}
static int hgic_iwpriv_set_icmp_mntr(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    ctrl->icmp_mntr = simple_strtol(data, 0, 10);
    return hgic_fwctrl_set_icmp_mntr(ctrl, ifidx, ctrl->icmp_mntr);
}
static int hgic_iwpriv_set_passwd(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    return hgic_fwctrl_set_passwd(ctrl, ifidx, data);
}
static int hgic_iwpriv_set_rpasswd(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    return hgic_fwctrl_set_rpasswd(ctrl, ifidx, data);
}
static int hgic_iwpriv_set_throughput(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
    ctrl->throughput = val > 3 ? 3 : val;
    return hgic_fwctrl_set_throughput_en(ctrl, ifidx, val);
}
static int hgic_iwpriv_set_mem_recycle(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
    return hgic_fwctrl_set_mem_recycle(ctrl, ifidx, val);
}
static int hgic_iwpriv_set_host_monitor(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
    return hgic_fwctrl_set_host_monitor(ctrl, ifidx, val);
}
static int hgic_iwpriv_set_dma_scatter(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
    return hgic_fwctrl_set_dma_scatter(ctrl, ifidx, val);
}
...
static int hgic_iwpriv_set_edca_max(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    struct hgic_txq_param txq;
    memcpy(&txq, data, sizeof(txq));
    return hgic_fwctrl_set_edca_max(ctrl, ifidx, &txq);
}
static int hgic_iwpriv_set_slot_time(struct hgic_fwctrl *ctrl, u8 ifidx, char *data, u32 count)
{
    int val = simple_strtol(data, 0, 10);
    return hgic_fwctrl_set_slot_time(ctrl, ifidx, val);
}
...
static struct fwctrl_cfgset hgpriv_sets[] = {
    ...
    {"pool_maxusage",               hgic_iwpriv_set_pool_maxusage},
    {"icmp_mntr",                   hgic_iwpriv_set_icmp_mntr},
    {"passwd",                      hgic_iwpriv_set_passwd},
    {"rpasswd",                     hgic_iwpriv_set_rpasswd},
    {"throughput",                  hgic_iwpriv_set_throughput},
    {"mem_recycle",                 hgic_iwpriv_set_mem_recycle},
    {"host_mntr",                   hgic_iwpriv_set_host_monitor},
    {"dma_scatter",                 hgic_iwpriv_set_dma_scatter},
    {"sleep_roaming",               hgic_iwpriv_set_sleep_roaming},
    {"slot_time",                   hgic_iwpriv_set_slot_time},
    {"edca_max",                    hgic_iwpriv_set_edca_max},
    {NULL,}
};
...
static int hgic_iwpriv_get_chip_uuid(struct hgic_fwctrl *ctrl, u8 ifidx, struct iwreq *wrqin)
{
    char format = 0;
    int ret;
    char *ptr;
    ...
    ret = hgic_copyfrom_iwreq(wrqin, buff, sizeof(buff));
    ptr = strchr(buff, '=');
    if (ptr) {
        format = simple_strtol(ptr + 1, 0, 10);
    }
    
    ret = hgic_fwctrl_get_chip_uuid(ctrl, ifidx, uuid);
    if (ret == sizeof(uuid)) {
        if(format == 1){
            memset(buff, 0, sizeof(buff));
            for(ret=0;ret<sizeof(uuid);ret++) sprintf(buff+2*ret, "%02x", uuid[ret]);
            hgic_copyto_iwreq(wrqin, buff, strlen(buff));
        }else{
            hgic_copyto_iwreq(wrqin, (char *)uuid, sizeof(uuid));
        }
        return (0);
    }
    return ret;
...
static struct fwctrl_cfgget hgpriv_gets[] = {
    ...
    {"uuid",                        hgic_iwpriv_get_chip_uuid},
    {NULL,}
};
...
int hgic_iwpriv_dump(struct hgic_fwctrl *ctrl, struct iwreq *wrqin)
{
    ...
    struct fwctrl_cfgget *get = NULL;
    struct fwctrl_cfgset *set = NULL;
    int buflen = wrqin->u.data.length;
    ...
    if (buf) {
        ...
        for (set = hgpriv_sets; set->name && len + 32 < buflen; set++) {
            len += snprintf(buf + len, buflen - len, "  %s\r\n", set->name);
        }
        len += snprintf(buf + len, buflen - len, "get:\r\n");
        for (get = hgpriv_gets; get->name && len + 32 < buflen; get++) {
            len += snprintf(buf + len, buflen - len, "  %s\r\n", get->name);
        }
        hgic_copyto_iwreq(wrqin, buf, len);
        kfree(buf);
