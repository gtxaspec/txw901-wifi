...
#define FWDATA_MAX_SIZE    (512*1024)
...
void hgic_release_firmware(const struct firmware *fw, u8 cust_path)
{
    if (cust_path) {
        if (fw && fw->data) {
            kfree(fw->data);
...
int hgic_request_firmware(const struct firmware **fw, const char *name, struct device *device)
{
    if (strchr(name, '/')) {
        int len;
        struct file *fp = NULL;
        struct firmware *_fw = kzalloc(sizeof(struct firmware), GFP_KERNEL);

        if (_fw) {
            _fw->data = (const u8 *)kmalloc(FWDATA_MAX_SIZE, GFP_KERNEL);
        }

        if (_fw && _fw->data) {
            fp = filp_open(name, O_RDONLY, 0);
            if (!IS_ERR(fp)) {
                len = _KERNEL_READ(fp, (char *)_fw->data, FWDATA_MAX_SIZE);
                filp_close(fp, NULL);
                ...
                hgic_err("open %s fail\r\n", name);
            }
        }else{
            hgic_err("request_firmware fail, no mem\r\n");
        }

        hgic_release_firmware(_fw, 1);
        return -ENOMEM;
    } else {
...
static int hgic_bootdl_parse_fw(struct hgic_bootdl *hg, const char *fw_name)
{
    ...
    hg->cust_path = (strchr(fw_name, '/') != NULL);
    ret = hgic_request_firmware(&fw, fw_name, hg->ctrl->dev);
    if (ret) {
        hgic_err("request_firmware failed, ret:%d [%s]\n", ret, fw_name);
        return ret;
    }

    hgic_dbg("request firmware %s, size=%d\n", fw_name, fw->size);
    ...
    hg->fw_info.run_addr = hgic_get_fw_run_addr(fw->data, &err_code);
    if (err_code) {
        hgic_dbg("hgic_get_fw_dl_addr error!\n");
        return err_code;
    }
    ...
    hg->fw_info.version     = hgic_get_fw_boot_need_reset(fw->data, &err_code);//1:both;0:need reset
    if (err_code) {
        hgic_dbg("hgic_get_fw_bootreset error!\n");
        return err_code;
    }
    hg->fw_info.fw_len     = fw->size - hg->fw_info.hdr_len;

    hgic_dbg("firmware hdr len  : %d\r\n", hg->fw_info.hdr_len);
    hgic_dbg("firmware load addr : %x\r\n", hg->fw_info.write_addr);
    hgic_dbg("firmware run addr : %x\r\n", hg->fw_info.run_addr);
    hgic_dbg("firmware size     : %d\r\n", hg->fw_info.fw_len);
    hgic_dbg("firmware aes_en:%d,crc_en:%d,version:%x\r\n",
             hg->fw_info.aes_en, hg->fw_info.crc_en, hg->fw_info.version);
...
static int hgic_bootdl_cmd_run(struct hgic_bootdl *hg)
{
    ...
    if (skb) {
        cmd_hdr = (struct hgic_bootdl_cmd_hdr *)skb->data;
        put_unaligned_le32(hg->fw_info.run_addr, cmd_hdr->addr);

        temp = hg->fw_info.fw_len;
        if (hg->fw_info.aes_en) {
            temp  |= HG_BOOT_CMD_RUN_PREACT_AES_DEC;
        }
        if (hg->fw_info.crc_en && hg->fw_info.write_addr == hg->fw_info.run_addr) {
            temp  |= HG_BOOT_CMD_RUN_PREACT_CRC_CHK;
        }
        if (hg->fw_info.version == 0) {
            temp  |= HG_BOOT_CMD_RUN_PREACT_SP_CFG;// boot reset
        }
...
int hgic_bootdl_download(struct hgic_bootdl *hg, const char *fw_path)
{
    ...
    if (ret) {
        hgic_dbg("firmware run failed, err:%d\r\n", ret);
    } else {
        hgic_dbg("firmware run success!\n");
    }

__finish:
    if (hg->fw) {
        hgic_dbg("Release boot download firmware...\n");
        hgic_release_firmware(hg->fw, hg->cust_path);
        hg->fw = NULL;
    }
    hgic_leave();
    return ret;
