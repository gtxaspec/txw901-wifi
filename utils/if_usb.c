...
#ifndef USB_DMA_ALIGN
#define USB_DMA_ALIGN (4)
#endif
...
static int hgic_usb_qinit(struct hgic_usb *usb, struct list_head *q, int qsize, u8 rx)
{
    ...
    for (i = 0; i < qsize; i++) {
        ...
        if (buf->urb) {
            if (rx) {
                buf->rxbuff = kmalloc(max_pkt_len + USB_DMA_ALIGN + 4, GFP_KERNEL);
                if (buf->rxbuff == NULL) {
                    usb_free_urb(buf->urb);
                    kfree(buf);
                ...
                buf->data = PTR_ALIGN(buf->rxbuff, USB_DMA_ALIGN);
...
static void hgic_usb_tx_complete(struct urb *urb)
{
    ...
    if(buf->txbuff) kfree(buf->txbuff);
    buf->data = NULL;
    buf->txbuff = NULL;
...
static int hgic_usb_tx_packet(void *bus, struct sk_buff *skb)
{
    ...
    if(PTR_ALIGN(txbuff, USB_DMA_ALIGN) != txbuff){
        //hgic_err("data addr is unaligned! %p, %d\r\n", txbuff, USB_DMA_ALIGN);
        ptr = kmalloc(len + USB_DMA_ALIGN, GFP_KERNEL);
        if(ptr == NULL){
        ...
        txbuff = PTR_ALIGN(ptr, USB_DMA_ALIGN);
    ...
    usb_fill_bulk_urb(buf->urb, usbdev->udev, usb_sndbulkpipe(usbdev->udev, usbdev->ep_out),
                      txbuff, len, hgic_usb_tx_complete, buf);
