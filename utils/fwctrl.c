...
static void hgic_fwctrl_tx_work(struct work_struct *work)
{
    struct sk_buff *skb = NULL;
    struct hgic_ctrl_hdr *hdr = NULL;
    struct hgic_fwctrl *ctrl = container_of(work, struct hgic_fwctrl, tx_work);

    do{
        skb = skb_dequeue(&ctrl->txq);
        if (skb) {
            ...
            switch (hdr->hdr.type) {
                ...
                case HGIC_HDR_TYPE_BLUETOOTH:
                    hdr->hdr.cookie = ctrl->data_cookie(ctrl->priv);
                default:
                    if (test_bit(HGIC_BUS_FLAGS_INBOOT, &ctrl->bus->flags)) {
                        kfree_skb(skb);
                        skb = NULL;
                    }
                    break;
        ...
        if (skb) {
            ...
            ctrl->bus->tx_ctrl(ctrl->bus, skb);
        }
    }while(!skb_queue_empty(&ctrl->txq));
}

static void hgic_fwctrl_rx_work(struct work_struct *work)
{
    struct hgic_fwctrl *ctrl = NULL;
    struct sk_buff *skb = NULL;
    struct hgic_ctrl_hdr *hdr = NULL;

    ctrl = container_of(work, struct hgic_fwctrl, rx_work);
...
struct sk_buff *hgic_fwctrl_send_data(struct hgic_fwctrl *ctrl, struct sk_buff *skb, struct hgic_cmd_response *resp, u32 timeout)
{
    ...
    queue_work(ctrl->tx_wq, &ctrl->tx_work);
    if (timeout) {
        if (!wait_for_completion_timeout(&resp->cmpl, msecs_to_jiffies(timeout))) {
            hgic_err("timeout, ctrl->rxq:%d, ctrl->txq:%d\r\n", skb_queue_len(&ctrl->rxq), skb_queue_len(&ctrl->txq));
        }
    }
    spin_lock_irqsave(&ctrl->lock, flags);
    list_del(&resp->list);
...
void hgic_fwctrl_rx(struct hgic_fwctrl *ctrl, u8 *data, int len)
{
    ...
    if(hgic_is_enter_sleep(data)){
        hgic_err("device sleep RESP ...\r\n");
        set_bit(HGIC_BUS_FLAGS_SLEEP_RESP, &ctrl->bus->flags);
    }
    ...
    if (skb) {
        ...
        queue_work(ctrl->rx_wq, &ctrl->rx_work);
    } else {
...
void hgic_fwctrl_init(struct hgic_fwctrl *ctrl, void *dev, struct hgic_bus *bus)
{
    ...
    INIT_WORK(&ctrl->rx_work, hgic_fwctrl_rx_work);
    INIT_WORK(&ctrl->tx_work, hgic_fwctrl_tx_work);
    ctrl->rx_wq = ALLOC_ORDERED_WORKQUEUE("fwctrl_rx", 0);
    ctrl->tx_wq = ALLOC_ORDERED_WORKQUEUE("fwctrl_tx", 0);
    BUG_ON(ctrl->rx_wq == NULL || ctrl->tx_wq == NULL);
}

void hgic_fwctrl_release(struct hgic_fwctrl *ctrl)
{
    if(ctrl->bus == NULL) return;
    cancel_work_sync(&ctrl->rx_work);
    cancel_work_sync(&ctrl->tx_work);
    if (ctrl->rx_wq) {
        flush_workqueue(ctrl->rx_wq);
        destroy_workqueue(ctrl->rx_wq);
    }
    if (ctrl->tx_wq) {
        flush_workqueue(ctrl->tx_wq);
        destroy_workqueue(ctrl->tx_wq);
    }
...
int hgic_fwctrl_get_fwinfo(struct hgic_fwctrl *ctrl, u8 ifidx, struct hgic_fw_info *info)
{
    ...
    if (ret > 0) {
        ...
        info->app_version = le32_to_cpu(_info.app_version);
...
int hgic_fwctrl_set_coexist_en(struct hgic_fwctrl *ctrl, u8 ifidx, u8 data[2])
{
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_COEXIST_EN, data, 2);
}
...
int hgic_fwctrl_set_pool_maxusage(struct hgic_fwctrl *ctrl, u8 ifidx, u8 maxusage[2])
{
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_POOL_MAXUSAGE, maxusage, 2);
}

int hgic_fwctrl_set_icmp_mntr(struct hgic_fwctrl *ctrl, u8 ifidx, u8 en)
{
    return hgic_fwctrl_set_byte(ctrl, ifidx, HGIC_CMD_SET_ICMP_MNTR, en);
}

int hgic_fwctrl_set_passwd(struct hgic_fwctrl *ctrl, u8 ifidx, u8 *passwd)
{
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_WIFI_PASSWD, passwd, strlen(passwd));
}

int hgic_fwctrl_set_rpasswd(struct hgic_fwctrl *ctrl, u8 ifidx, u8 *passwd)
{
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_WIFI_RPASSWD, passwd, strlen(passwd));
}

int hgic_fwctrl_get_chip_uuid(struct hgic_fwctrl *ctrl, u8 ifidx, u8 uuid[12])
{
    return hgic_fwctrl_get_bytes(ctrl, ifidx, HGIC_CMD_GET_CHIP_UUID, uuid, 12);
}

int hgic_fwctrl_set_throughput_en(struct hgic_fwctrl *ctrl, u8 ifidx, u8 en)
{
    return hgic_fwctrl_set_byte(ctrl, ifidx, HGIC_CMD_SET_THROUGHPUT_EN, en);
}

int hgic_fwctrl_set_mem_recycle(struct hgic_fwctrl *ctrl, u8 ifidx, u8 en)
{
    return hgic_fwctrl_set_byte(ctrl, ifidx, HGIC_CMD_SET_MEM_RECYCLE, en);
}

int hgic_fwctrl_set_host_monitor(struct hgic_fwctrl *ctrl, u8 ifidx, u8 en)
{
    return hgic_fwctrl_set_byte(ctrl, ifidx, HGIC_CMD_SET_HOST_MONITOR, en);
}

int hgic_fwctrl_set_dma_scatter(struct hgic_fwctrl *ctrl, u8 ifidx, u32 size)
{
    return hgic_fwctrl_set_int_val(ctrl, ifidx, HGIC_CMD_SET_DMA_SCATTER, size);
}

int hgic_fwctrl_set_ble_llpkt_len(struct hgic_fwctrl *ctrl, u8 ifidx, u32 length)
{
    return hgic_fwctrl_set_int_val(ctrl, ifidx, HGIC_CMD_SET_BLE_LLPKT_LEN, length);
}
...
int hgic_fwctrl_set_edca_max(struct hgic_fwctrl *ctrl, u8 ifidx, struct hgic_txq_param *param)
{
    return hgic_fwctrl_set_bytes(ctrl, ifidx, HGIC_CMD_SET_EDCA_LIMIT_MAX, (u8 *)param, sizeof(struct hgic_txq_param));
}

int hgic_fwctrl_set_slot_time(struct hgic_fwctrl *ctrl, u8 ifidx, u32 slot_time)
{
    return hgic_fwctrl_set_int_val(ctrl, ifidx, HGIC_CMD_SET_EDCA_SLOT_TIME, slot_time);
}
